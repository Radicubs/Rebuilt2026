...
public class SwerveModule implements Sendable {
    ...
    public void setDesiredState(SwerveModuleState desiredState, boolean isOpenLoop) {
        ...
        if (SwerveConstants.useMagicMotion) {
    ...
    private void setSpeed(SwerveModuleState desiredState, boolean isOpenLoop) {
        if (isOpenLoop) {
            ...
            double driveVel = Conversions.MPSToRPS(desiredState.speedMetersPerSecond, SwerveConstants.wheelCircumference);
    ...
    public SwerveModuleState getState() {
        return new SwerveModuleState(
                Conversions.RPSToMPS(mDriveMotor.getVelocity().getValue().in(Units.RotationsPerSecond), SwerveConstants.wheelCircumference),
    ...
    public SwerveModulePosition getPosition() {
        return new SwerveModulePosition(
                Conversions.rotationsToMeters(mDriveMotor.getPosition().getValue().in(Units.Rotation), SwerveConstants.wheelCircumference),
                Rotation2d.fromRotations(mAngleMotor.getPosition().getValue().in(Units.Rotation))
        );
    }
}